...
public class FlightControllerNew {
	...
	//plane shot down thingy(port from LabJac)
	...
	public void SetAxes(EntityPlane plane)
	{
		...
		float groundSpeed = (float)plane.getSpeedXZ();
		...
		if(mode == EnumPlaneMode.PLANE)
		{
			...
			if(!plane.isPartIntact(EnumDriveablePart.rightWing))
				roll += 20F * speed;
			if (speed > 2.0F && type.supersonic == false){
				pitch -= 50 * speed;
			}
	...
	public void PlaneModeFly(EntityPlane plane)
	{
		...
		//Unit conversions
		float speed = (float)plane.getSpeedXYZ();
		float pitch = plane.axes.getPitch();
		float roll = plane.axes.getRoll();
		float yaw = plane.axes.getYaw();
		Vector3f longAxis = (Vector3f)plane.axes.getXAxis().normalise();
		Vector3f vertAxis = (Vector3f)plane.axes.getYAxis().normalise();
		Vector3f horiAxis = (Vector3f)plane.axes.getZAxis().normalise();		
		...
		if (thrust/(type.mass*5F) > type.mass) {
			lift = gravity;
		}
