...
public class BlockFlansWorkbench extends Block
{
    ...
    {
	if (i == 1) return (j >= 0 && j <= 2) ? top[j] : side;
	return side;
    }
	...
	if (metadata == 0 && isRemote) guiId = 0;
	else if (metadata == 1 && !isRemote) guiId = 2;
	if (guiId != -1) entityplayer.openGui(FlansMod.INSTANCE, guiId, world, i, j, k); 	
	return true;
    }
